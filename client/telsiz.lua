...
RegisterNUICallback('telsiz', function(data, cb)
    ...
    if tonumber(data.channel) ~= tonumber(getPlayerRadioChannel) then
        if tonumber(data.channel) <= Config.OzelKanal then
...
RegisterNUICallback('telsizterket', function(data, cb)
   local playerName = GetPlayerName(PlayerId())
   local getPlayerRadioChannel = exports.tokovoip_script:getPlayerData(playerName, "radio:channel")
    if getPlayerRadioChannel == "nil" then
        ESX.ShowNotification("Bir kanalda değildin")
    --  exports['mythic_notify']:SendAlert('inform', Config.messages['not_on_radio'])
        else
          exports.tokovoip_script:removePlayerFromRadio(getPlayerRadioChannel)
          exports.tokovoip_script:setPlayerData(playerName, "radio:channel", "nil", true)
    --      exports['mythic_notify']:SendAlert('inform', Config.messages['you_leave'] .. getPlayerRadioChannel .. '.00 MHz </b>')
    ESX.ShowNotification("Kanaldan çıktın")
end
...
RegisterCommand('telsizgir', function()
  ...
  aramadurumu = aramadurumu - 1
  else
    ESX.ShowNotification("Zaten bir aramadasın")
  end
end)



-- RegisterNUICallback('aramayap', function(data)
--   TriggerServerEvent('arayan:kisiyeilet')
--   ESX.ShowNotification("ARAMAA MORUQQ")
--   aranan = data.aranan
--   ESX.ShowNotification('aranan' ..aranan.. 'numara')
--   arayangir()
--   TriggerServerEvent('aranan:bildirim', aranan)
-- end)
...
RegisterNUICallback('aramakapat', function(data)
  aramakapat()
  PhonePlayAnim('out')
  PhonePlayIn()
end)
...
RegisterNUICallback('aramayap', function(data)
  PhonePlayCall()
  arayangir(data.aranan)
 -- TriggerServerEvent('arama:bildirim', data.aranan)
  TriggerServerEvent('arama:geldibildirim', data.aranan)
  --TriggerServerEvent("arama:geldi", GetPlayerServerId(data.aranan))

end)

function arayangir(data)
  local _source = source
  local PlayerData = ESX.GetPlayerData(_source)
  local playerName = GetPlayerName(PlayerId())
  local getPlayerRadioChannel = exports.tokovoip_script:getPlayerData(playerName, "radio:channel")
  local seskanali =  math.random(Config.CallStartRoom, Config.CallStopRoom)
  local channel = seskanali
  ...
  exports.tokovoip_script:addPlayerToRadio(tonumber(channel))
  aramadurumu = aramadurumu - 1
  TriggerServerEvent('arama:gittibildirim', data.aranan)
  else
  --  ESX.ShowNotification("Zaten bir aramadasın")
    TriggerServerEvent('arama:mevcud')
  end
...
function aramakapat()
  ...
  local playerName = GetPlayerName(PlayerId())
  local getPlayerRadioChannel = exports.tokovoip_script:getPlayerData(playerName, "radio:channel")
  if aramadurumu == 1 then 
  exports.tokovoip_script:removePlayerFromRadio(getPlayerRadioChannel)
  TriggerServerEvent('arama:kapatmabildirim')
  aramadurumu = aramadurumu + 1
  else 
    TriggerServerEvent('arama:degilsin')
   -- ESX.ShowNotification("Bir aramada değilsin")
  end
end
